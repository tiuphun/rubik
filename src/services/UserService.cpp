...
void UserService::signUp(const string& username, const string& password) {
    // // Hash the password
    ...
    // if (rc != SQLITE_OK) {
    //     return {{"status", "error"}, {"message", sqlite3_errmsg(db)}};
    // }
...
void UserService::signIn(const string& username, const string& password) {
    // // Hash the password
    // unsigned char hash[SHA256_DIGEST_LENGTH];
    // SHA256((unsigned char*)password.c_str(), password.length(), hash);
    // string password_hash(reinterpret_cast<char*>(hash), SHA256_DIGEST_LENGTH);
